...
public class MicrophoneListener : MonoBehaviour {
	...
	//Values Jose Moyano
	public float soundValue;                          //Numeric output for use in other scripts.
    public int experienceDuration;                    //Time of experience.
    public float sensitivity = 850f;                  //Micro sensibility.
    public static MicrophoneListener microSingleton; 
    //

    //Written in part by Benjamin Outram and Jose Moyano
	...
	void MicrophoneIntoAudioSource (bool MicrophoneListenerOn){      
		if(MicrophoneListenerOn){
			...
			if (Time.time - timeSinceRestart > 0.5f && !Microphone.IsRecording (null)) {
				src.clip = Microphone.Start (null, true, experienceDuration, 44100); //The third value is the duration of audio experience             
